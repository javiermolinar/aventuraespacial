...
export function BuildActivity({ config, state, sound, onEarn, onPlace, onComplete }: {
  config: BuildConfiguration; state: BuildState; sound: boolean;
  onEarn: () => void; onPlace: () => void; onComplete: () => void;
}) {
  ...
  return <MotionConfig reducedMotion="user"><div className={`robot-lab adventure-build ${state.ready ? 'piece-ready' : ''}`}>
    <div className="workshop-grid">
      <Factory placed={parts.slice(0, state.placedCount).map(part => part.id)} selected={selected.id} ready={state.ready} color={config.robot.color} design={config.robot.design} complete={state.placedCount === 6} activeParts={parts} onPlace={onPlace} />
      <section className="activity-panel" aria-label="Construye a tu compañera">
        {reached ? <div className="completion-panel"><span className="celebration-icon"><Check size={42} /></span><h2>{`¡Has construido a ${config.robot.name}!`}</h2><p>{config.completion}</p><button className="primary" onClick={onComplete}>Seguir la historia<ArrowRight size={21} /></button></div> : state.ready ? <div className="earned-panel"><span className="success-label"><Check size={22} />{operationLabel(operation)} = {resultOf(operation)}</span><Hand className="drag-instruction-icon" size={62} /><h2>Coloca {selected.name}</h2><p>Arrastra la pieza o tócala y después toca su silueta.</p></div> : <MathPuzzle key={state.placedCount} operation={operation} partName={selected.name} sound={sound} onSolved={onEarn} />}
      </section>
    </div>
