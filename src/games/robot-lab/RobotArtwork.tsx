...
export function PartShape({ id, color, design }: { id: PartId; color: string; design: RobotDesign }) {
  const ink = '#34384b';
  switch (id) {
    case 'head': return <>
...
export function PartOutline({ id, design }: { id: PartId; design: RobotDesign }) {
...
export const robotPartOrder: PartId[] = ['leftLeg', 'rightLeg', 'leftArm', 'rightArm', 'body', 'head'];

/** Static portraits share the game's geometry without loading its animation runtime. */
export function RobotPortrait({ color, design, complete = true }: { color: string; design: RobotDesign; complete?: boolean }) {
  return <svg viewBox="0 0 400 420" className={`robot-svg ${complete ? 'is-complete' : ''}`} aria-label={complete ? 'Robot terminado' : 'Plano del robot'} role="img">
    ...
    <g>{robotPartOrder.map(id => <g key={id}><PartShape id={id} color={color} design={design} /></g>)}</g>
  </svg>;
}
