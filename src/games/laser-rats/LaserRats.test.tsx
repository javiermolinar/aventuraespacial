...
it('pairs discovery, rat surprises, lasers and wins with distinct effects and music', () => {
  ...
  expect(cell('B3').querySelector('.laser-reveal-pulse')).toBeTruthy();
...
it('moves keyboard focus without scanning and cancels robot mode with Escape', () => {
  ...
  expect(next.className).toContain('is-fog');
...
it('keeps discovered-cell counts visible across selection changes and updates them after firing', () => {
  const { container } = render(<LaserRats />);
  const cell = (id: string) => container.querySelector<HTMLButtonElement>(`[data-cell="${id}"]`)!;
  ...
  fireEvent.click(cell('C3'));
  expect([...cell('C3').querySelectorAll('.laser-direction-count')].map(el => el.textContent)).toEqual(['↑1', '↓1']);
  expect(cell('C2').querySelector('.laser-direction-count')).toBeNull();
  expect(cell('C4').querySelector('.laser-direction-count')).toBeNull();
  expect(cell('C2').className).toContain('is-fog');
  expect(container.querySelectorAll('.laser-direction-count')).toHaveLength(2);
  expect(container.querySelector('.laser-radar-panel')).toBeNull();
  fireEvent.click(cell('C2'));
  expect(cell('C2').className).toContain('is-known');
  expect([...cell('C3').querySelectorAll('.laser-direction-count')].map(el => el.textContent)).toEqual(['↑1', '↓1']);
  expect([...cell('C2').querySelectorAll('.laser-direction-count')].map(el => el.textContent)).toEqual(['↑1', '←1', '→1', '↓1']);
  fireEvent.click(screen.getByRole('button', { name: 'Colocar robot de columnas' }));
  fireEvent.click(cell('C3'));
  expect(cell('C3').querySelector('.laser-direction-count')).toBeNull();
  expect([...cell('C2').querySelectorAll('.laser-direction-count')].map(el => el.textContent)).toEqual(['←1', '→1']);
  expect(cell('B2').className).toContain('is-fog');
  expect(cell('B2').querySelector('.laser-direction-count')).toBeNull();
