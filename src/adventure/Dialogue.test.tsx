...
it('keeps the passage across questions, resets answer state, and resumes the current question', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: 'En la estación Luna.' }));
  ...
  cleanup();
  render(<Adventure chapter={chapter} />);
  expect(await screen.findByRole('heading', { name: '¿Funciona la radio?' })).toBeTruthy();
  fireEvent.click(screen.getByRole('button', { name: 'No.' }));
  fireEvent.click(screen.getByRole('button', { name: 'Continuar' }));
  expect(screen.getByRole('button', { name: 'Entrar al taller' })).toBeTruthy();
});

it('can go back and forward with blocked storage without resetting the run', async () => {
  ...
  render(<Adventure chapter={chapter} />);
  await screen.findByRole('heading', { name: 'Mejor en compañía' });
  fireEvent.click(screen.getByRole('button', { name: 'Volver atrás' }));
...
it('uses the same question actions in illustrated robot dialogue, then advances to another page', async () => {
  const script: ChapterScript = structuredClone(chispaScript);
  ...
  expect(await screen.findByRole('heading', { name: 'Chispa' })).toBe(document.activeElement);
