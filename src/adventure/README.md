...
`chapters/brote.ts` publishes chapter 2 under the existing `brote` catalogue ID. “¡Qué hambre!” follows the next morning's flight to estación Luna, the search for food, six-operation construction, Brote lowering the boxes, the 5 × 5 six-piece “El gran cuadrado” packing puzzle, and breakfast as the final scene. Its opening uses dedicated `journey` artwork for the hungry cockpit scene and `storage` artwork for the food discovery, each with boy/girl and portrait/landscape WebP variants in `public/adventure/chapters/brote/`. Later scenes retain the existing cockpit compositions and Brote SVG introduction, avoiding the hungry expression after the snack. Chapter version 3 removes the sequencing recap and merges the ending into breakfast. Version 1/2 saves retain story/build progress and map old closing scenes to `snack`; version 2 keeps its 5 × 5 placements, while version 1's 3 × 3 placements are discarded.
