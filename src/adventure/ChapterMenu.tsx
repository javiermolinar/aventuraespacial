...
export function ChapterMenu({ catalog, campaign, onSelect, practiceHref = '../practice.html' }: {
  catalog: readonly ChapterEntry[]; campaign: CampaignProgress; onSelect: (id: string) => void; practiceHref?: string;
}) {
  return <section className="chapter-menu" aria-labelledby="chapter-menu-title">
    ...
    <ol className="chapter-list">{catalog.map((entry, index) => {
      ...
      return <Fragment key={entry.id}><li>
      ...
      </li>{index === 0 && <li><a className="chapter-card practice-card" href={practiceHref} aria-label="Practicar mates">
        <span className="chapter-number">Práctica libre</span>
        <span className="chapter-robot" aria-hidden="true"><Calculator size={60} /></span>
        <span className="chapter-name">Practicar mates</span>
        <span className="chapter-card-action">Jugar<ArrowRight size={16} aria-hidden="true" /></span>
      </a></li>}</Fragment>;
    })}</ol>
  </section>;
}
