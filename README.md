...
- `/games/robot-lab.html?level=1`: the maths-only robot factory. Levels are numbered 1–11.
...
5. A solved operation releases a part onto a moving conveyor in both practice and story games. The catch window follows the selected maths level: eight seconds at level 1, half a second less per level, down to three seconds at level 11. The belt and rollers speed up with the piece. Catch it by dragging, tapping, or pressing Enter/Space, then place it on the matching robot outline at your own pace. Incorrect drops return to the belt with the remaining time. A missed piece tumbles onto the floor beside the belt and stays available to drag, tap or pick up with the keyboard. The solved operation and every placed part remain earned; an incorrect drop of a floor piece returns it to the floor. The timer pauses in hidden tabs and while rereading the story; reduced motion keeps the piece still and shows the countdown. Resuming a saved story gives a pending piece a fresh trip across the belt.
...
Levels 8–11 extend practice to hundreds: mixed three-digit operations without
regrouping, addition carrying across units and tens, subtraction borrowing across
columns (including zeros), and three-addend sums carrying **2**. For example,
`289 + 178 + 177 = 644` carries 2 twice. The child enters each full column total,
then confirms carrying its tens to the next column. Results stay below 1,000.
Earlier levels and saved completion indices are unchanged. The new robots include
Cobalto, Nova and Titan; the final challenge builds an expert Rayo variant.
...
**El chef del tiempo** is a standalone practice game with a robot chef. Each menu contains four recipes: toast with a visible explanatory hint, then pancakes, cake, and soup without hints. There is no lightbulb button; only the first dish shows a hint, including on retries. Each recipe has one fixed input clock, alternating analog, digital, analog, digital; there is no separate clock playground. The four levels cover whole hours, half hours, quarter past/to, and arbitrary minutes. The last level generates fresh non-five-minute targets when entering or restarting the menu; input changes and retries preserve them. Any level can be selected; completing one offers the next.

The visible reminder **1 hora = 60 minutos** connects a full minute-hand revolution to an hour. Hints explain the minute marks and show the matching 12-hour and 24-hour clocks, with a shaded minute sector. **¡Listo!** first animates the recipe’s clock toward the submitted answer—moving hands for analog recipes or advancing digits for digital recipes—with a minutes counter: 08:30 demonstrates 08:00–08:30, while 08:00 demonstrates a full 60-minute turn from 07:00. This is a fast demonstration of clock movement, not elapsed cooking time or a countdown. Reduced-motion preferences skip the animation. Early answers then show raw food; late answers show cartoon smoke. Feedback identifies a wrong minute, hour, or half-day; retries keep the hint restricted to the first dish. Correct answers advance only when the child chooses. All recipe targets are on the same calendar day. No lives or saved progress; sound uses the existing preference.

Drag either hand directly, or focus it and use keyboard arrows; there are no hand-selection or clock-format mode buttons. The minute hand carries the hour hand across hours, noon, and midnight. The final level supports one-minute dragging and keyboard steps; no extra minute buttons are added to the analog clock. The digital alarm uses accessible increment/decrement buttons with the same precision. Two narrow vertical timelines flank the analog clock: twelve labeled hour slots (00–11) on the left and twelve (12–23) on the right. Sun/sunset icons and light-to-dark colors place the hours in a day. One visible marker follows the day through the first twelve hours, then the second twelve. Completed hour slots remain shaded while future slots stay empty. The rails are display-only: dragging a clock hand updates the day position and highlights the current hour. Turning the hour hand once reaches the other half of the day. There are no morning/afternoon selector buttons. These rails also track the accelerated clock demonstration. Purple consistently means hours and teal means minutes. Escape, pointer cancellation, or lost capture restores the pre-drag time. Phones stack the recipe and controls; the animated clock scrolls into view when submitted.
...
The five soundtrack recordings are by [Kevin MacLeod (incompetech.com)](https://incompetech.com/), licensed [Creative Commons Attribution 4.0](https://creativecommons.org/licenses/by/4.0/): **Wallpaper**, **Dream Culture**, **Carefree**, **Life of Riley** and **Cipher**. [Full music attribution](public/music/ATTRIBUTION.txt) includes original source URLs, hashes and recording changes. Recordings are served locally as 96 kbps MP3s to reduce download size. Nunito uses the SIL Open Font License; its license is distributed at `public/licenses/Nunito-OFL.txt`.
